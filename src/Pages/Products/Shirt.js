...
function Shirt() {
  ...
  const getData = (e) => {
    ...
    axios.post("http://localhost:2022/prize/range",pk).then((data) => {
      console.log(data)
      setShirtData(data?.data?.response?.result)
    }).catch((err) => {
      console.log(err)
    })
  }
    ...
    <div>
      ...
      <div className='container'>
        <div className='row'>
          ...
          <div className='col s3 center'>
            <select id='range' className="browser-default style13" name='range' onChange={(e) => setrange(e.target.value)} required>
              <option >All Prices</option>
              <option value={V1} >less then 600</option>
              <option value={V2} >less then 900</option>
              <option value={V3}>less 2000</option>
            </select>
          </div>
          <div className='col s3 center'>
